...
function updateMenuDisplay() {
    ...
    if (window.innerWidth <= 1024) {
        menuItems.forEach(item => {
            item.style.display = 'none'; // 메뉴 항목 숨김
        });
        subBtn.style.display = 'inline-block'; // 참석문의 버튼 보이기
    } else {
        menuItems.forEach(item => {
...
// 스크롤 이벤트 리스너 추가
window.addEventListener('scroll', initializeNavbar);
...
document.querySelector('.menu-icon').addEventListener('click', function() {
    ...
    slidingMenu.classList.toggle('open'); // 슬라이딩 메뉴의 클래스 토글 (열고 닫기)
});

// 닫기 버튼 클릭 이벤트 리스너 추가
document.querySelector('.close-btn').addEventListener('click', function() {
    const slidingMenu = document.querySelector('.sliding-menu');
    slidingMenu.classList.remove('open'); // 팝업 창 닫기
});
